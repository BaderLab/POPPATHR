...
propDir <- sprintf("%s/data/geneProperties", dataDir)
outDir  <- sprintf("%s/res/%s_out_selEnrichGenePropertyIntegration", dataDir, dt)
...
sink(sprintf("%s/selEnrichGenePropertyIntegration.log", outDir))
...
gwasF_out <- sprintf("%s/nhgri_gwas_hits.txt", propDir)
...
drugF_out <- sprintf("%s/drug_gene_interactions.txt", propDir)
...
omimF_out <- sprintf("%s/omim_morbidmap.txt", propDir)
...
eqtlF_out <- sprintf("%s/gtex_eqtl_V7", propDir)
...
if (file.exists(eqtlF_out)) {
  ...
  system(sprintf("tar -zxvf %s.tar.gz -C %s --strip-components 1",
        eqtlF_out, eqtlF_out))
...
###############################################################################
### Check formatting for each file separately, then query each gene for
### gene properties
cat("\nFILE STATS\n")
## GWAS hits
gwas <- read.delim(gwasF_out, h=T, as.is=T)

# Subsetting for studies done in both European and African populations
pops <- paste(c("*European.*African*", "*African.*European"), collapse="|")
gwas <- gwas[grep(pops, gwas$INITIAL.SAMPLE.SIZE),]
# Order df by trait
gwas <- gwas[order(gwas$MAPPED_TRAIT),]

# Split delimited gene names column `REPORTED.GENE.S.` and insert as new rows (duplicate)
gwas2 <- gwas[,c(35,14,22)]
gene_split <- strsplit(gwas2$REPORTED.GENE.S., ", ")
#gwas_df <- data.frame(gwas_mapped_trait=rep(gwas2$MAPPED_TRAIT, sapply(gene_split, length)),
#                      snp=rep(gwas2$SNPS, sapply(gene_split, length)),
#                      gene=unlist(gene_split))
gwas_df <- data.frame(gwas_mapped_trait=rep(gwas2$MAPPED_TRAIT, sapply(gene_split, length)),
                      gene=unlist(gene_split))
trait_split <- strsplit(as.character(gwas_df$gwas_mapped_trait), ", ")
gwas_df <- data.frame(gwas_mapped_trait=unlist(trait_split),
                       gene=rep(gwas_df$gene, sapply(trait_split, length)))
gwas_df <- unique(gwas_df)
cat(sprintf(paste("GWAS\n",
                  "* %i unique gene-trait associations (European/African samples only)\n",
                  "* %i genes + %i traits\n"),
      nrow(gwas_df), length(unique(gwas_df[,2])), length(unique(gwas_df[,1]))))
...
omim_df <- data.frame(omim_phenotype=rep(omim_pheno,
                                     sapply(omim_split, length)),
...
geneDir <- sprintf("%s/res/out_180412_HM3_pops_hg19_CEU-YRI_FST_10-300gene_500kb-dist_10000perm_updated_data/ld/hc_snps", dataDir)
gene <- read.delim(sprintf("%s/genes_leadingEdge_unique_hc.txt", geneDir), h=F, as.is=T)
colnames(gene) <- "gene"

cat("*Getting pathway cluster info\n")
clust <- read.delim(sprintf("%s/gseaStat_per_hc_pathway_updated.txt", geneDir), h=T, as.is=T)
clust <- clust[,-c(3:5)]
colnames(clust)[3] <- "pathway_cluster"
...
getGeneProperties <- function(prop_list) {
  for (i in seq_along(prop_list)) {
    ...
    # Aggregate multiple rows based on common gene names
...
# Combine data for all queries and write to excel sheet
all_prop <- join_all(dfs=geneProp_list, by="gene", type="full")
all_prop <- all_prop[order(all_prop$gene),]
write.table(all_prop, sprintf("%s_pathwayCluster_all.txt", outF), col=T, row=F, quote=F, sep="\t")
