...
#'
#' @param inFile (char) path to PLINK bim file (only CHR, SNP, and BP
#' columns considered).
#' @param refgeneFile (char) path to refseq table with header.
#' @param marg (integer) region upstream and downstream of [txStart,txEnd] (default=0).
#' @param outputFile (char) path to write snp-to-gene mapping file.
#' @export
SNP2gene <- function(inFile, refgeneFile, marg=0, outputFile) {
	# Read in SNP table
	cat(sprintf("* Reading SNP file: %s\n", inFile))
	snps <- fread(inFile, header=FALSE, data.table=FALSE)
	snp_GR <- GRanges(
		paste("chr", snps[,1], sep=""),
		IRanges(snps[,4], snps[,4]),
		name=snps[,2]
	)
	refGene <- read.delim(refgeneFile, sep="\t", header=TRUE, as.is=TRUE)
	gene_GR	<- GRanges(
		refGene[,"chrom"],
	...
	d_in <- data.frame(queryHits=d0@from, subjectHits=d0@to, distance=dbit)
	idx <- which(dbit == 0)
	out1 <- cbind(
	...
	write.table(out, file=outputFile, sep="\t", col=FALSE, row=FALSE, quote=FALSE)
}
